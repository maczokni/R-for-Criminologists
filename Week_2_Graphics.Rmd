---
title: 'Week 2 Graphics with ggplot2'
...
# WEEK 2: Graphics with ggplot2

**Table of Contents**

- [Section 2.1: Introduction](#introduction)
- [Section 2.2: Anatomy of a plot](#anatomy-of-a-plot)
- [Section 2.3: What graph should I use?](#what-graph-should-I-use?)
- [Section 2.4: Histograms](#histograms)
- [Section 2.5: Density plots](#density-plots)
- [Section 2.6: Box plots](#box-plots)
- [Section 2.7: Scatter plots with two variables](#scatter-plots-with-two-variables)
- [Section 2.8 Scatter plots conditioning in a third v variable](#scatter-plots-conditioning-in-a-third-variable)
- [Section 2.9: Scatterplot matrix](#scatterplot-matrix)
- [Section 2.10: Titles and legends in ggplot2](#titles-and-legends-in-ggplot2)
- [Section 2.11: Plotting categorical data: bar charts](#plotting-categorical-data-bar-charts)
- [Section 2.12: Further resources](#further-resources)
...
![](week2imgs/layers.png)

This will result in a final plot: 

![](week2imgs/combined.png)

Let's have a look at what this looks like for a graph. 
<!-- To get the data load up the package "MASS".

```{r}
library(MASS)
```

This package has a dataframe called "Boston". This has data about Housing Values in Suburbs of Boston. To access the codebook (how you find out what variables are) use the "?". 

OK so let's make a graph about the variable which represents the per capita crime rate by town ('crim').

```{r}

ggplot(data = Boston, aes(x = crim, y=medv)) +          #data
   geom_point() +                           #geometry
...
The second line is where we add the geometry. This is where we tell R what we want the graph to be. Here we say we want it to be a histogram. You can see a list of all possible geoms [here](http://docs.ggplot2.org/current/). 
...
OK what if we don't want it to be a histogram, we want it to be a density curve?

```{r}

ggplot(data = Boston, aes(x = crim)) +          #data
   geom_density() +                               #geometry
  theme_bw()                                    #backgroud coordinate system

```

Cool! But what if I like both?
...
ggplot(data = Boston, aes(x = crim)) +        #data
geom_histogram(aes(y=..density..)) +          #geometry 1
geom_density()+                               #geometry 2
theme_bw()                                    #backgroud coordinate system
...
The classic book is by Edward Tufte, but since him there are many other researchers as well who focus on approaches to displaying data. 
...
This makes the comparison significantly easier, right? But the order is strange! Remember we talked about factors last week? Well the good thing about factors is that we can arrange them in their natural order. If we dont' describe an order, then R uses the alphabetical order. So let's reorder our factor:
...
This can be even more helpful when you have various groups. Let's try an example using the `BCS0708` data frame we introduced last week. If you did not save your project after last week and need to load the data again. 
...
##2.7 Scatter plots with two variables
...
##2.8 Scatter plots conditioning in a third variable
...
##2.10 Titles and legends in ggplot2
...
##2.11 Plotting categorical data: bar charts
...
There are other tools sometimes used for visualising categorical data. Pie charts is one of them. However, as mentioned at the beginning, many people advocate strongly against using pie charts, including Juanjo who wrote this course before me, and he explicitly stats that they are evil, and therefore this is the only pie chart you will see in this course:
